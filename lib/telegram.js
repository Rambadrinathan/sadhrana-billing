 ...
 */
export function purchaseKeyboard({ showAdopt = false } = {}) {
...
/**
 * Attendance roster: one button per staff member (tap cycles their status),
 * two per row to stay readable on a phone, plus bulk actions and save.
 */
export function rosterKeyboard(entries = []) {
  ...
  for (let i = 0; i < entries.length; i += 2) {
    rows.push(
      entries.slice(i, i + 2).map((e, j) => ({
        text: `${MARK[e.status] || "●"} ${String(e.name).slice(0, 12)}`,
        callback_data: `rost:cycle:${i + j}`,
      }))
    );
  }
  rows.push([
    { text: "✔ All present", callback_data: "rost:all" },
    { text: "↺ Reload", callback_data: "rost:reload" },
  ]);
  rows.push([
    { text: "✅ Save day", callback_data: "rost:save" },
    { text: "❌ Cancel", callback_data: "cancel" },
  ]);
  return { inline_keyboard: rows };
}
