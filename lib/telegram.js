...
export function confirmKeyboard(
  ...
  cardFee = false,
  cardFeePct = 2.5,
  // Corporate F&B: same as room invoices — buyer company + GSTIN on the PDF
  // so the customer can claim input credit. Off by default (most guests are B2C).
  buyerGstin = null
) {
  const rows = [];
  ...
  // Same option as the room invoice web form. Without the buyer's GSTIN on the
  // face of the F&B tax invoice, a corporate guest cannot claim input credit.
  const gstinShort = buyerGstin
    ? String(buyerGstin).trim().toUpperCase().slice(0, 15)
    : "";
  rows.push([
    gstinShort
      ? {
          text: `🏢 GSTIN ${gstinShort} — tap to remove`,
          callback_data: "b2b:off",
        }
      : {
          text: "🏢 Company GSTIN? tap to add (like room invoice)",
          callback_data: "b2b:on",
        },
  ]);
  rows.push([
...
*Billing*
*/bill* — then slip photo or typed items → Confirm → PDF
...
🏢 *Company GSTIN* — same as room invoices: tap to bill a company so their
GSTIN prints on the F&B tax invoice (for input credit).
