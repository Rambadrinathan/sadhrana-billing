...
export function confirmKeyboard(gstApplied = true) {
  const gstLabel = gstApplied
    ? "🧾 GST: ON (5%) — tap to turn OFF"
    : "🧾 GST: OFF — tap to turn ON";
  return {
    inline_keyboard: [
      [{ text: gstLabel, callback_data: gstApplied ? "gst:off" : "gst:on" }],
      [
...
*GST picker on every draft*
Tap *GST: ON/OFF* before confirm (5% = 2.5%+2.5% when ON).

*Edit existing*
/edit SB-2026-0003
then send full corrected bill text, then yes.

*Commands*
/menu · /today · /edit · /history · /pdf · /paid · /cancel · /help`;
