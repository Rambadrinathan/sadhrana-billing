...
async function previewAndStore(chatId, draft) {
  ...
  const text = editNote + gstNote + formatDraftPreview(enriched, totals);
  const kb = confirmKeyboard(enriched.gst_applied !== false);
  try {
    await sendMessage(chatId, text, { reply_markup: kb });
  } catch {
    await sendPlain(
      chatId,
      text.replace(/\*/g, "") + "\n\nReply YES to confirm or NO to cancel. Tap GST button to toggle.",
      { reply_markup: kb }
    );
  }
}
  ...
  try {
    ...
    if (draft.edit_bill_id) {
      result = await editBill(draft.edit_bill_id, {
        ...
        change_note: draft.change_note || "Edited via Telegram",
      });
    } else {
