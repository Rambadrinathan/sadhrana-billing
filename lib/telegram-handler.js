...
async function previewAndStore(chatId, draft) {
  ...
  const staffNote = enriched.created_by
    ...
    : `\n👤 *Select staff* (or *Others* if not on the list)\n`;
...
async function sendInvoicePdf(chatId, bill, pdfBuffer, extraCaption = "", replyMarkup = null) {
  ...
  const cap = [
    ...
    `Total ₹${Number(bill.grand_total).toLocaleString("en-IN")} (incl. GST)`,
...
async function confirmDraft(chatId) {
  ...
  if (!String(draft.created_by || "").trim()) {
    await sendPlain(
      chatId,
      "Select *which staff* is creating this bill (tap a name, or *Others* and type their name), then Confirm again."
    );
...
async function handleCommand(chatId, text) {
  ...
  if (c === "/attendance") {
    ...
    await sendRosterCard(chatId, todayIst());
    return;
  }
...
async function handleText(chatId, text) {
  ...
  // Attendance roster open: "3 in 10:30" / "3 out 17:00" / "save"
  if (pending?.kind === "roster") {
    if (["cancel", "stop"].includes(lower)) {
      await clearDraft(chatId);
      await sendPlain(chatId, "Attendance draft cancelled — nothing saved.");
      return;
    }
    if (["save", "done", "ok"].includes(lower)) {
      await saveRoster(chatId);
      return;
    }
    const m = t.match(/^(\d+)\s*(in|out)\s*([0-2]?\d[:.][0-5]\d)$/i);
    if (m) {
      const idx = Number(m[1]) - 1;
      const field = m[2].toLowerCase();
      const time = m[3].replace(".", ":").padStart(5, "0");
      await applyRosterChange(chatId, (entries) => {
        const e = entries[idx];
        if (!e) return `There's no ${m[1]} on the list.`;
        if (e.status !== "present" && e.status !== "half") {
          return `${e.name} is marked ${e.status} — tap them to Present first.`;
        }
        e[field] = time;
        return `${e.name}: ${field} ${time}`;
      });
      return;
    }
    await sendRich(
      chatId,
      "Attendance is open. Tap a name to change status, or send:\n" +
        "`3 in 10:30`  ·  `3 out 17:00`  ·  `save`  ·  `cancel`"
    );
    return;
  }
  ...
  // Free-typed attendance: "in Munish" / "out Munish"
  const attM = t.trim().match(/^(in|out)\s+(.+)$/i);
  if (attM) {
    const name = attM[2].trim().slice(0, 40);
    try {
      if (attM[1].toLowerCase() === "in") {
        const res = await clockIn({ staffName: name, chatId });
        await sendPlain(chatId, res.message);
      } else {
        const res = await clockOut({ staffName: name, chatId });
        await sendPlain(chatId, res.message);
      }
    } catch (e) {
      await sendPlain(
        ...
        "Attendance error: " +
          (e.message || e) +
          "\n(Run SETUP_OPS.sql if tables missing.)"
      );
    }
    return;
  }
    ...
    if (e?.code === "NOT_PURCHASE" || e?.message === "NOT_PURCHASE") {
      await sendPlain(
        ...
        "📷 That doesn't look like a *supplier purchase* invoice.\n" +
          ...
          "For guest bills, send the slip without /purchase.\n" +
          "For purchases: /purchase then a clear GST invoice photo."
      );
      return;
    }
...
export async function handleUpdate(update) {
  if (update.callback_query) {
    ...
    // Attendance roster: rost:cycle:<i> | rost:all | rost:reload | rost:save
    if (data.startsWith("rost:")) {
      ...
      if (part === "reload") {
        await answerCallback(cq.id, "Reloading");
        await sendRosterCard(chatId, todayIst());
        return;
      }
      if (part === "all") {
        await answerCallback(cq.id, "All present");
        await applyRosterChange(chatId, (entries, { defaultTimes }) => {
          for (const e of entries) {
            e.status = "present";
            Object.assign(e, defaultTimes("present"));
          }
          return "All marked present.";
        });
        return;
      }
      if (part.startsWith("cycle:")) {
        const idx = Number(part.slice(6));
        await answerCallback(cq.id, "Changed");
        await applyRosterChange(chatId, (entries, { nextStatus, defaultTimes }) => {
          const e = entries[idx];
          if (!e) return "";
          e.status = nextStatus(e.status);
          Object.assign(e, defaultTimes(e.status));
...
/* ------------------------------------------------------------------ *
 * Telegram attendance roster — Munish marks the whole team.
 * Writes the same daily rows as the web page.
 * ------------------------------------------------------------------ */

/** Roster card: one tappable row per staff member, plus bulk + save. */
async function sendRosterCard(chatId, date, note = "") {
  const { getDayRoster } = await import("@/lib/attendance");
  const { STATUS_MARK, STATUS_LABEL, summaryLine } = await import(
    "@/lib/attendance-roster"
  );

  let entries;
  try {
    entries = await getDayRoster(date);
  } catch (e) {
    await sendPlain(chatId, "Could not load roster: " + (e.message || e));
    return;
  }
  if (!entries.length) {
  ...
  // Draft holds the working state until Save
  const draft = { kind: "roster", date, entries };
  await saveDraft(chatId, draft);

  const rows = entries.map((e, i) => {
    const times =
      e.status === "present" || e.status === "half" ? ` ${e.in}-${e.out}` : "";
    const flag = e.saved ? "" : " ·new";
    return `${i + 1}. ${STATUS_MARK[e.status]} *${e.name}* — ${STATUS_LABEL[e.status]}${times}${flag}`;
  });

  const text =
    `🕒 *Attendance · ${date}*\n\n${rows.join("\n")}\n\n` +
    `${summaryLine(entries)}\n\n` +
    `Tap a name to cycle Present → Absent → Half → Leave.\n` +
    `Times: send \`3 in 10:30\` or \`3 out 17:00\`\n` +
    (note ? `\n${note}\n` : "");

  await sendRich(chatId, text, {
    reply_markup: rosterKeyboard(entries),
  });
}

/** Applies one roster change held in the draft, then redraws. */
async function applyRosterChange(chatId, mutate, note = "") {
  const d = await getDraft(chatId);
  if (!d || d.kind !== "roster") {
    await sendPlain(chatId, "No attendance draft open. Send /attendance.");
    return;
  }
  const { nextStatus, defaultTimes } = await import("@/lib/attendance-roster");
  const entries = d.entries.map((e) => ({ ...e }));
  const msg = mutate(entries, { nextStatus, defaultTimes });
  await saveDraft(chatId, { ...d, entries });
  await sendRosterCard2(chatId, d.date, entries, note || msg || "");
}

/** Redraw without re-reading the DB (keeps unsaved edits visible). */
async function sendRosterCard2(chatId, date, entries, note = "") {
  const { STATUS_MARK, STATUS_LABEL, summaryLine } = await import(
    "@/lib/attendance-roster"
  );
  const rows = entries.map((e, i) => {
    const times =
      e.status === "present" || e.status === "half" ? ` ${e.in}-${e.out}` : "";
    return `${i + 1}. ${STATUS_MARK[e.status]} *${e.name}* — ${STATUS_LABEL[e.status]}${times}`;
  });
  const text =
    `🕒 *Attendance · ${date}*\n\n${rows.join("\n")}\n\n` +
    `${summaryLine(entries)}\n` +
    (note ? `\n${note}\n` : "") +
    `\n_Not saved yet_ — tap ✅ Save day.`;
  await sendRich(chatId, text, { reply_markup: rosterKeyboard(entries) });
}

async function saveRoster(chatId) {
  const d = await getDraft(chatId);
  if (!d || d.kind !== "roster") {
    await sendPlain(chatId, "No attendance draft open. Send /attendance.");
    return;
  ...
  try {
    const { saveDayAttendance } = await import("@/lib/attendance");
    const { summaryLine } = await import("@/lib/attendance-roster");
    const res = await saveDayAttendance({
      date: d.date,
      entries: d.entries,
      markedBy: d.marked_by || "Telegram",
      source: "telegram",
    });
    await clearDraft(chatId);
    await sendRich(
      chatId,
      `✅ *Attendance saved · ${d.date}*\n${res.saved} staff recorded.\n\n${summaryLine(d.entries)}`
    );
  } catch (e) {
    await sendPlain(chatId, "Could not save attendance: " + (e.message || e));
  }
}
