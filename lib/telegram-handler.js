...
import { storePurchaseFile, storeBillPhoto } from "@/lib/purchase-store";
import {
  getCatalog,
  createBill,
  computeTotals,
...
import { saveDraft, getDraft, clearDraft, claimOnce } from "@/lib/drafts";
import {
...
/**
 * Parse company + GSTIN (+ optional address) from one Telegram message.
 * Accepts either line-broken or "Company | GSTIN | address" form.
 */
function parseBuyerDetailsMessage(text) {
  const raw = String(text || "").trim();
  if (!raw) return { ok: false, error: "Send the company name and GSTIN." };
  ...
  if (raw.includes("|")) {
    ...
    // Single-line fallback: last 15-char GSTIN-looking token
    if (!gstin && lines.length === 1) {
      const m = raw.match(/\b([0-9]{2}[A-Z0-9]{13})\b/i);
      if (m) {
        gstin = m[1];
        company = raw.replace(m[0], "").replace(/[|\-–—,]+/g, " ").trim();
      }
    }
  }
  ...
  if (!company || company.length < 2) {
    return {
      ok: false,
      error:
        "Need the *company name* (Buyer — Bill to).\n\nSend like:\n`Acme Pvt Ltd`\n`07AABCU9603R1ZP`\n`Delhi` _(address optional)_",
    };
  }
  ...
  if (!v.ok || v.empty) {
    return {
      ok: false,
      error: v.empty
        ? "Need the 15-character *customer GSTIN* on the next line (or after |)."
        : v.error,
    };
  }
  return {
    ok: true,
    buyer_company: company.slice(0, 120),
    buyer_gstin: v.gstin,
    buyer_address: address ? address.slice(0, 200) : null,
...
async function prepareBillDraft(draft) {
  ...
  if (enriched.gst_inclusive === undefined) enriched.gst_inclusive = false;
  // Off unless Munish taps it: most bills are settled cash or UPI, and a fee
  // added by default would be charged to guests who never used a card.
  if (enriched.card_fee === undefined) enriched.card_fee = false;
...
/**
 * One line telling the operator whether the PAPER made it into storage.
 * Never silent on failure: an entry whose photo was rejected looks identical to
 * one that saved cleanly, which is exactly how every purchase before 2026-08-05
 * ended up with no evidence behind it.
 */
function paperNote(url, error) {
  if (url) return "\n📎 Photo attached to this entry.\n";
  if (error) {
    return (
      `\n⚠️ *Photo NOT saved* — ${error}\n` +
      "The entry will still save, but with no paper behind it.\n"
    );
  }
  return "";
}
...
/**
 * The three GST states a bill can be in, spelled out on the review card.
 *
 * INCLUDED exists because Munish is handed F&B bills whose amount is already
 * what the guest paid. Making him work backwards to the pre-tax figure is a
 * subtraction that goes wrong; he keys 6,000 and the invoice says 6,000.
 */
const GST_OFF_NOTE = "\n🧾 *GST: OFF* (no 5% tax on this invoice)\n";
const GST_ON_NOTE = "\n🧾 *GST: ON* (5% = CGST 2.5% + SGST 2.5%, added on top)\n";
/**
 * Card fee note. Spelled out with both figures because the guest will compare
 * the machine slip against the invoice, and Munish has to be able to answer
 * "why is it more than the bill?" without doing the arithmetic himself.
 */
const money2 = (n) =>
...
const CARD_FEE_NOTE = (grandTotal, fee, payable, pct) =>
  `\n💳 *Card payment: +${String(pct).replace(/\.0+$/, "")}% fee*\n` +
  `Bill ₹${money2(grandTotal)} + fee ₹${money2(fee)} = ` +
  `guest pays *₹${money2(payable)}*.\n` +
  "The fee is charged on the total after GST and is not taxed again.\n";

const GST_INCL_NOTE = (grandTotal) =>
  "\n🧾 *GST: INCLUDED in the amounts* — nothing is added.\n" +
  `Guest pays *₹${Number(grandTotal || 0).toLocaleString("en-IN")}*, the figure on the bill. ` +
  "The invoice still shows the taxable value and CGST/SGST inside it.\n";
...
async function previewAndStore(chatId, draft) {
  ...
  const totals = computeTotals(enriched.lines, {
    ...
    inclusive: enriched.gst_inclusive === true,
    cardFeePct: enriched.card_fee === true ? PROPERTY.cardFeePct : 0,
  });
  ...
  const gstNote =
    enriched.gst_applied === false
      ? GST_OFF_NOTE
      : enriched.gst_inclusive === true
        ? GST_INCL_NOTE(totals.grand_total)
        : GST_ON_NOTE;
  const staffNote =
    `\n👤 *Billed by: ${enriched.created_by}*\n` +
    paperNote(enriched.source_photo_url, enriched.photo_store_error);
  ...
  if (enriched.from_photo) {
    ...
    // Inclusive mode inverts the comparison: the figure on the paper is the
    // guest-facing TOTAL, so it must be checked against the grand total.
    // Comparing it to the (now net-of-tax) subtotal would report a false 5%
    // shortfall on every such bill.
    const compareVal = enriched.gst_inclusive === true ? grand : computedSub;
    if (written > 0 && Math.abs(written - compareVal) > 1) {
      const diff = Math.abs(written - compareVal);
      // Direction matters: over-billing a guest is worse than under-billing
      const dir = compareVal > written ? "OVER by" : "UNDER by";
      checkNote +=
        `\n⚠️ *ITEMS DO NOT MATCH THE SLIP*\n` +
        `Slip items total: *₹${written.toLocaleString("en-IN")}*\n` +
        `This bill: *₹${compareVal.toLocaleString("en-IN")}*\n` +
        `*${dir} ₹${diff.toLocaleString("en-IN")}* — check quantities and rates.\n`;
    } else if (written > 0) {
      checkNote +=
        `\n✅ Items match the slip (₹${written.toLocaleString("en-IN")}).` +
        (enriched.gst_inclusive === true
          ? ` GST is inside that figure — guest pays *₹${grand.toLocaleString("en-IN")}*.\n`
          : grand > computedSub
  ...
  // Same as room invoices: company + GSTIN must appear on the F&B tax invoice
  // for a corporate guest to claim input credit.
  const buyerNote = enriched.buyer_gstin
    ? `\n🏢 *Bill to:* ${enriched.buyer_company || "—"}\n` +
      `GSTIN: *${enriched.buyer_gstin}*` +
      (enriched.buyer_address ? `\n${enriched.buyer_address}` : "") +
      `\n`
    : "";
  ...
  const text =
    ...
    taxNote +
    checkNote +
    formatDraftPreview(enriched, totals);
...
async function sendInvoicePdf(chatId, bill, pdfBuffer, extraCaption = "", replyMarkup = null) {
  ...
  const cap = [
    ...
    // The caption is what gets read before the PDF is opened, so a card bill
    // has to state the figure the guest will actually be charged.
    Number(bill.card_fee_inr) > 0
      ? `💳 + card fee ₹${money2(bill.card_fee_inr)} = *₹${money2(amountPayable(bill))}* payable`
      : null,
...
async function sendPurchaseReview(chatId, draft) {
  ...
  const head =
    ...
    // Totals come from the items, so the header can't disagree with the list.
    // With GST off (the default) there is no tax line to show at all.
    (draft.gst_applied
      ? `Items Rs ${inr(linesTotals(draft.lines).subtotal)} · GST ${PURCHASE_GST_PCT}% Rs ${inr(linesTotals(draft.lines).gst)} · ` +
        `*Total Rs ${inr(linesTotals(draft.lines).total)}*`
      : `*Expense Rs ${inr(linesTotals(draft.lines).subtotal)}* _(no GST)_`) +
    (draft.total_manual ? " _(corrected)_" : "") +
    "\n";

  // Default is no GST, but don't hide it when the paper clearly shows tax —
  // that would understate a formal supplier bill.
  const gstHint =
    !draft.gst_applied && Number(draft.ocr_gst_inr) > 0
      ? `\nℹ️ This invoice looks like it shows GST of Rs ${inr(draft.ocr_gst_inr)}. ` +
        `If it's a formal GST bill, tap *No GST* below to add ${PURCHASE_GST_PCT}%.\n`
      : "";
  ...
  const body =
    ...
    gstHint +
    paperNote(draft.invoice_pdf_url, draft.photo_store_error);

  const kb = purchaseKeyboard({
    showAdopt: m.hasLines && m.mismatch,
    gstOn: Boolean(draft.gst_applied),
  });
...
async function confirmDraft(chatId) {
  ...
  if (draft.awaiting_buyer_details) {
    await sendPlain(
      chatId,
      "Still waiting for company name + GSTIN.\n" +
        "Send them now, or reply *cancel* to bill the guest only."
    );
    return;
  }
  ...
  try {
    ...
    const gstInclusive = gstApplied && draft.gst_inclusive === true;
    const cardFee = draft.card_fee === true;
    ...
    if (draft.edit_bill_id) {
      result = await editBill(draft.edit_bill_id, {
        ...
        card_fee: cardFee,
        buyer_company: draft.buyer_company || null,
      ...
      const bill = await createBill({
        ...
        source_photo_url: draft.source_photo_url || null,
        source_photo_path: draft.source_photo_path || null,
        buyer_company: draft.buyer_company || null,
        buyer_gstin: draft.buyer_gstin || null,
        buyer_address: draft.buyer_address || null,
      });
...
async function startEdit(chatId, billNo) {
  ...
  const draft = {
    ...
    notes: bill.notes,
    buyer_company: bill.buyer_company || null,
    buyer_gstin: bill.buyer_gstin || null,
    buyer_address: bill.buyer_address || null,
...
async function handleText(chatId, text) {
  ...
  // Company + GSTIN after tapping "Company GSTIN?" on an F&B draft — same fields
  // the room invoice form collects, so the PDF can print Buyer GSTIN.
  if (pending?.awaiting_buyer_details && hasBillIntent(pending)) {
    if (["no", "n", "cancel", "nah", "stop"].includes(lower)) {
      pending.awaiting_buyer_details = false;
      await saveDraft(chatId, pending);
      await sendPlain(chatId, "Cancelled. Bill stays guest-only (no company GSTIN).");
      await previewAndStore(chatId, pending);
      return;
    }
    const parsed = parseBuyerDetailsMessage(t);
    if (!parsed.ok) {
      await sendPlain(chatId, parsed.error + "\n\nOr reply *cancel*.");
      return;
    }
    pending.buyer_company = parsed.buyer_company;
    pending.buyer_gstin = parsed.buyer_gstin;
    pending.buyer_address = parsed.buyer_address;
    pending.awaiting_buyer_details = false;
    await saveDraft(chatId, pending);
    await sendPlain(
      chatId,
      `🏢 Bill to *${parsed.buyer_company}* · GSTIN *${parsed.buyer_gstin}*` +
        (parsed.stateName ? ` (${parsed.stateName})` : "") +
        `\nConfirm the bill when ready.`
    );
    await previewAndStore(chatId, pending);
    return;
  }
...
async function handlePhoto(chatId, message) {
  ...
  //
  // Fifteen vouchers were once sent as a single album. The model cannot tell
  // where one voucher ends and the next begins, the drafts overwrote each
  // other, and nothing survived to be confirmed — so the whole batch had to be
  // re-entered and the operator reasonably believed the app had eaten it.
  //
  // The fix is a refusal, not better parsing. One photo, read it back, confirm
  // it, then the next. That is slower per voucher and far faster per evening.
  if (message.media_group_id) {
    const groupKey = `album:${chatId}:${message.media_group_id}`;
    // Only the first update of the album speaks; the rest are silent, or the
    // operator gets one refusal per photo.
    if (await claimOnce(groupKey)) {
      await sendPlain(
        chatId,
        "🛑 *Several photos at once — I have not read any of them.*\n\n" +
          "I can only do one piece of paper at a time. Fifteen vouchers in one " +
          "go cannot be told apart, and nothing would get saved.\n\n" +
          "*Send the first voucher on its own.* I will read it back to you, you " +
          "confirm it, then send the next one."
      );
    }
    return;
  }

  // A photo arriving on top of an unconfirmed draft is the same mistake made
  // one message at a time. Answering the one in hand first is what makes the
  // batch survive.
  // Careful: "await_bill" and "await_purchase_photo" mean the bot ASKED for a
  // photo and is waiting for it, so those must sail straight through. Only a
  // draft that has already been read back and is waiting on a yes/no blocks.
  const awaitingConfirmation =
    existing?.kind === "purchase" ||
    (hasBillIntent(existing) && existing?.kind !== "await_bill");
  if (awaitingConfirmation) {
    await sendPlain(
      chatId,
      "⏳ *One at a time.* There is still an entry waiting for you.\n\n" +
        "Send *confirm* to save it, or *cancel* to drop it — then send this photo again."
    );
    return;
  }
  const wantPurchase =
  ...
  // Neutral wording. Naming the mode ("purchase invoice") reads as a claim about
  // what the photo IS, which is confusing when the guess turns out wrong.
  await sendPlain(chatId, "Analyzing the invoice…");
  ...
  try {
    ...
    if (wantPurchase) {
      ...
      const draft = {
        ...
        photo_store_error: stored.error || null,
      };
      // GST OFF by default: daily purchases from local vendors carry no tax.
      draft.gst_applied = false;
      draft.lines = applyPurchaseGst(draft.lines, false);
      const t0 = linesTotals(draft.lines);
      draft.amount_inr = t0.subtotal;
      draft.gst_amount_inr = 0;
      draft.total_inr = t0.subtotal;
      draft.ocr_gst_inr = Number(purchase.gst_amount_inr) || 0;
      await saveDraft(chatId, draft);
    ...
    // Keep the slip itself. The generated invoice PDF is our transcription of it,
    // not evidence of it — if a guest disputes a line, the paper is what settles it.
    const slip = await storeBillPhoto(buffer, {
      filename: `slip-${Date.now()}.jpg`,
      contentType: mimeType || "image/jpeg",
    });
    draft.source_photo_url = slip.url;
    draft.source_photo_path = slip.path;
    draft.photo_store_error = slip.error || null;
    if (message.caption) {
    ...
    if (wrongMode && heldFileId) {
      ...
      await sendRich(
        chatId,
        (looksLikeBill
          ? "📷 That reads as money coming *in*, not a purchase.\n"
...
export async function handleUpdate(update) {
  if (update.callback_query) {
    ...
    if (data === "purch:gston" || data === "purch:gstoff") {
      const on = data === "purch:gston";
      await answerCallback(cq.id, on ? `GST ${PURCHASE_GST_PCT}% added` : "GST removed");
      const d = await getDraft(chatId);
      ...
      d.gst_applied = on;
      d.gst_manual = true;
      d.lines = applyPurchaseGst(d.lines, on);
      const t = linesTotals(d.lines);
      d.amount_inr = t.subtotal;
      d.gst_amount_inr = t.gst;
      d.total_inr = t.total;
      await saveDraft(chatId, d);
    ...
    if (data === "gst:on" || data === "gst:off" || data === "gst:incl") {
      const draft = await getDraft(chatId);
      if (!draft) {
        await answerCallback(cq.id, "No draft");
        return;
      }
      // One button, three states: ON adds 5% on top, INCLUDED takes the 5% out
      // of what was keyed in (so the grand total stays exactly as written), OFF
      // charges no tax at all.
      draft.gst_applied = data !== "gst:off";
      draft.gst_inclusive = data === "gst:incl";
    ...
    if (data === "card:on" || data === "card:off") {
      const draft = await getDraft(chatId);
      if (!draft) {
        await answerCallback(cq.id, "No draft");
        return;
      }
      // Independent of the GST cycle: the fee is 2.5% of whatever the grand
      // total works out to, in all three tax states.
      draft.card_fee = data === "card:on";
      await answerCallback(
        cq.id,
        draft.card_fee
          ? `Card fee +${String(PROPERTY.cardFeePct).replace(/\.0+$/, "")}% added`
          : "Card fee removed"
      );
      await previewAndStore(chatId, draft);
      return;
    }
    // Company GSTIN on F&B — same option as the room invoice web form.
    if (data === "b2b:on" || data === "b2b:off") {
      const draft = await getDraft(chatId);
      if (!draft || !hasBillIntent(draft)) {
        await answerCallback(cq.id, "No bill draft");
        return;
      }
      if (data === "b2b:off") {
        draft.buyer_company = null;
        draft.buyer_gstin = null;
        draft.buyer_address = null;
        draft.awaiting_buyer_details = false;
        await saveDraft(chatId, draft);
        await answerCallback(cq.id, "Company GSTIN removed");
        await previewAndStore(chatId, draft);
        return;
      }
      // Ask for details in chat (Telegram has no inline form fields).
      draft.awaiting_buyer_details = true;
      await saveDraft(chatId, draft);
      await answerCallback(cq.id, "Send company + GSTIN");
      await sendPlain(
        chatId,
        "🏢 *Company booking* — same as room invoices.\n\n" +
          "Send in *one message* (2–3 lines):\n" +
          "`Company registered name`\n" +
          "`15-char GSTIN`\n" +
          "`Address` _(optional)_\n\n" +
          "Or: `Company | GSTIN | address`\n\n" +
