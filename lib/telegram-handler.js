...
async function previewAndStore(chatId, draft) {
  ...
  if (enriched.from_photo) {
    // A "5% GST" or "Total" row read as an item is what double-taxes the guest
    ...
    // Did the slip write its amounts with tax inside, or with tax on top?
    // The slip's own arithmetic decides — never ask Munish to remember.
    if (!enriched.gst_manual) {
      ...
      if (mode.applyGst === false) {
        // Amounts already include tax. Back it out so the invoice still shows
        // CGST/SGST and the grand total equals what the guest was shown.
        enriched.lines = backOutTax(enriched.lines, PROPERTY.defaultGstPct);
        enriched.gst_applied = true;
        taxNote =
          `\n🧾 *Tax was already in the slip amounts* — backed out so the ` +
          `invoice shows CGST/SGST and totals the same as the paper.\n`;
      } else if (mode.applyGst === true) {
        enriched.gst_applied = true;
  ...
  let checkNote = "";
  if (enriched.from_photo) {
    const written = Number(enriched.written_total_inr) || 0;
    const computed = Number(totals.grand_total) || 0;
    const badRows = (enriched.lines || []).filter((l) => l.row_mismatch);
    const rateRows = (enriched.lines || []).filter((l) => l.rate_differs);

    if (written > 0 && Math.abs(written - computed) > 1) {
      const diff = Math.abs(written - computed);
      // Direction matters: over-billing a guest is worse than under-billing
      const dir = computed > written ? "OVER by" : "UNDER by";
      checkNote +=
        `\n⚠️ *DOES NOT MATCH THE SLIP*\n` +
        `Slip total: *₹${written.toLocaleString("en-IN")}*\n` +
        `This bill: *₹${computed.toLocaleString("en-IN")}*\n` +
        `*${dir} ₹${diff.toLocaleString("en-IN")}* — check quantities and rates.\n`;
    } else if (written > 0) {
      checkNote += `\n✅ Matches the slip total (₹${written.toLocaleString("en-IN")}).\n`;
    } else {
