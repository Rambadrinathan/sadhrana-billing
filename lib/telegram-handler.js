...
async function enrichDraftWithCatalog(draft) {
  ...
  const lines = (draft.lines || []).map((l) => {
    ...
    if (!matched) {
      // Custom line: keep written rate, force 5% GST (F&B policy)
      return {
        ...l,
    ...
    // Official menu rate always wins — no guest negotiation on catalog items
    return {
...
async function confirmDraft(chatId) {
  ...
  try {
    ...
    // Professional tax invoice PDF (GSTIN, CGST/SGST, HSN) — sanctity
    try {
      const pdfBuf = await buildInvoicePdf(bill);
      ...
      await sendDocument(chatId, pdfBuf, fname, cap);
    } catch (pdfErr) {
      console.error("pdf", pdfErr);
      await sendPlain(chatId, "Bill saved but PDF failed: " + (pdfErr.message || pdfErr));
    }
