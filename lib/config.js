 ...
 * and MUST NOT be reworded — "Villa stay" or "Room charges" would read better
 * but would stop matching the accountant's ledger. If a line has to change,
 * it changes in Tally first.
 *
 * Place of supply is Haryana for both, so both are CGST + SGST, never IGST.
 * See lib/gstin.js → isIntraStateSupply for why.
 *
 * Numbering: lib/bill-no.js (CA series with underscores in the FY segment).
 */
export const INVOICE_KINDS = {
  restaurant: {
    ...
    cgstPct: 2.5,
    sgstPct: 2.5,
    // Full number looks like VJD/RS/26_27/031 — see allocateBillNo()
  ...
  accommodation: {
    ...
    cgstPct: 9,
    sgstPct: 9,
    // Full number looks like VJD/2026_27/036 — see allocateBillNo()
...
export const EXPENSE_CATEGORIES = [
  ...
  // Agent / booking commission we PAY out — money out, not guest revenue.
  { key: "commission", label: "Commission / agent" },
  { key: "other", label: "Other" },
];

/** Default GST on commission payouts (inclusive totals are backed out at this %). */
export const COMMISSION_GST_PCT = 18;
/** Informational SAC on settlement PDFs — confirm with CA if filing needs it. */
export const COMMISSION_SAC = "998559";
