...
import { invoiceKind, PROPERTY } from "@/lib/config";
import { allocateBillNo } from "@/lib/bill-no";
...
/**
 * The card fee in rupees for a given GST-inclusive total.
 *
 * Charged on the grand total — i.e. AFTER tax — because it recovers what the
 * bank takes on the whole swipe, and the bank swipes the tax too. It is not a
 * supply, so nothing is added to it: 2.5% flat, rounded to paise.
 */
export function cardFeeOn(grandTotal, pct) {
  const p = Number(pct) || 0;
  if (p <= 0) return 0;
  return round2(((Number(grandTotal) || 0) * p) / 100);
}

/**
 * What the guest actually hands over: the invoice total plus any card fee.
 *
 * `grand_total` deliberately stays the GST figure, so every revenue report,
 * the GST pack and the accountant's ledger keep reading the supply and only
 * the supply. The fee is a merchant cost passed through, never turnover — so
 * anything asking "what is owed / what did the machine take" asks THIS, and
 * anything asking "what did we earn" keeps asking grand_total.
 */
export function amountPayable(bill) {
  const grand = Number(bill?.grand_total) || 0;
  const fee = Number(bill?.card_fee_inr) || 0;
  return round2(grand + fee);
}
...
export function computeTotals(lines, opts = {}) {
  ...
  const cardFeePct = Number(opts.cardFeePct) || 0;
  ...
  const card_fee_inr = cardFeeOn(grand_total, cardFeePct);
  return {
    lines: computed,
    ...
    card_fee_pct: card_fee_inr > 0 ? cardFeePct : 0,
    card_fee_inr,
    // Grand total is the supply; payable is the supply plus the bank's cut.
    amount_payable: Math.round((grand_total + card_fee_inr) * 100) / 100,
  };
}
...
export async function createBill({
  ...
  // Guest is settling by card, so the bank's 2.5% is recovered on top of the
  // tax-inclusive total. Decided BEFORE the invoice is generated (the Telegram
  // review card / the web form), because the fee has to be printed on it — the
  // later `paid:…:card` button only records that the money arrived.
  card_fee = false,
  card_fee_pct = null,
}) {
  ...
  const {
    ...
    tax_total,
    grand_total,
    card_fee_inr: cardFeeInr,
    amount_payable: payable,
  } = computeTotals(rawLines, {
    applyGst,
    inclusive,
    cardFeePct: feePct,
  });
  ...
  let status = "unpaid";
  // Settled against what the guest owes, fee included — otherwise a card bill
  // paid in full reads as overpaid, and one paid to the pre-fee total reads as
  // settled while the fee is still outstanding.
  ...
  if (!isSupabaseConfigured()) {
    return {
      ...
      status,
      amount_paid: paid,
      payment_mode: status === "paid" ? (feePct > 0 ? "card" : "cash") : null,
  ...
  const supabase = getSupabase();
  // CA series (locked 2026-09): Stay VJD/2026_27/NNN · F&B VJD/RS/26_27/NNN
  // Continues after CA's last issued 035 (stay) and 030 (F&B).
  const billNo = await allocateBillNo(supabase, kind);
  ...
  if (status === "paid") {
    // A bill carrying a card fee was, by definition, settled on the machine.
    insert.payment_mode = feePct > 0 ? "card" : "cash";
  ...
  if (billErr) {
    const strip = [
      ...
      "buyer_state_name",
      "buyer_state_code",
      // Pre-migration safety net only. If these get stripped the fee is lost
      // from the row AND from the PDF (which is rendered from the saved row),
      // so the two never disagree — but apply the migration, don't rely on it.
      "card_fee_pct",
      "card_fee_inr",
    ];
...
export async function updateBillStatus(id, { status, payment_mode, payment_amount, payment_notes }) {
  ...
  // What the guest owes, card fee included — the figure on the machine slip.
  // Settlement is measured against this; revenue is still grand_total.
  const grand = amountPayable(current);
