...
/** Match free-text item names against catalog (fuzzy). */
export function matchCatalogItem(name, catalog) {
  ...
  for (const item of catalog || []) {
    ...
    if (iname.includes(n) || n.includes(iname)) {
      const score = Math.min(n.length, iname.length) / Math.max(n.length, iname.length);
      if (score > bestScore) {
        bestScore = score;
        best = item;
      }
    ...
    const it = iname.split(" ").filter(Boolean);
    const hit = it.filter((t) => nt.has(t) || [...nt].some((x) => x.startsWith(t) || t.startsWith(x)));
    if (hit.length >= 1) {
      const score = hit.length / Math.max(it.length, 1) + 0.1;
      if (score > bestScore) {
        bestScore = score;
        best = item;
      }
