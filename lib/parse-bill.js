...
/** Rupees with paise always shown — for figures that are rarely whole. */
function money2(n) {
...
export function formatDraftPreview(draft, totals) {
  ...
  return [
    ...
    draft.buyer_gstin
      ? `🏢 Bill to: *${escapeMd(draft.buyer_company)}* · GSTIN ${escapeMd(draft.buyer_gstin)}`
      : null,
    "",
    lines,
    "",
    ...
    // Only when a card fee is on: the total above stays the invoice figure, and
    // this is the figure the machine will take.
    // Paise matter here — a 2.5% fee lands on .50 more often than not, and
    // "₹262.5" on a payment instruction reads as a typo.
    totals.card_fee_inr
      ? `Card fee: ₹${money2(totals.card_fee_inr)}\n*Guest pays: ₹${money2(totals.amount_payable)}*`
      : null,
    "",
...
/**
 * Short follow-up after PDF (no storage / web links — PDF is the shareable invoice).
 */
export function formatBillMessage(bill, appUrl) {
  void appUrl;
  return [
    `✅ Invoice *${escapeMd(bill.bill_no)}* ready — PDF above.`,
    `${escapeMd(bill.guest_name)} · ${escapeMd(bill.villa)} · ₹${Number(
      bill.grand_total
    ).toLocaleString("en-IN")}`,
    `Status: ${bill.status}${bill.payment_mode ? " · " + bill.payment_mode : ""}`,
    "",
    "Forward the PDF on WhatsApp to the guest.",
    "To change items: /edit " + bill.bill_no,
  ].join("\n");
}
