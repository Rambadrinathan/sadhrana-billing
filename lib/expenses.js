...
/** Uppercased and trimmed, or null. Never throws — see the field comment. */
...
export async function createExpense(item) {
  ...
  const row = {
    ...
    // Both optional — most local purchases are from vendors with no GSTIN at
    // all. Recording them is what makes input GST claimable, so an empty one
    // is a fact about the purchase, not a validation failure.
    vendor_gstin: normalizeGstin(item.vendor_gstin),
    vendor_invoice_no: item.vendor_invoice_no
      ? String(item.vendor_invoice_no).trim().slice(0, 40)
      : null,
