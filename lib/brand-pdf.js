...
export function drawBrandHeaderBar(
  ...
  const titleSize = 14;
  text(title, 36, 841.89 - barH + 16, titleSize, true, BRAND.white);
  if (subtitle) {
    // MEASURE the title rather than guessing its width from character count.
    // `title.length * 8` under-estimated "TAX INVOICE" in bold, so the subtitle
    // was drawn on top of it and the header read "TAX INVOICESadhrana Bagh".
    let titleWidth;
    try {
      titleWidth = fontBold.widthOfTextAtSize(winAnsi(title), titleSize);
    } catch {
      titleWidth = title.length * 8.4; // fallback if the font can't measure
    }
    text(
      subtitle,
      36 + titleWidth + 10, // 10pt gutter so the two never touch
      841.89 - barH + 18,
      8,
      false,
      BRAND.sand
