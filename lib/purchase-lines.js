...
export function formatLinesReview(draft) {
  ...
  const rows = lines.map((l, i) => {
    const gstNote = Number(l.gst_pct) > 0 ? ` · GST ${l.gst_pct}%` : "";
    return (
      `*${i + 1}.* ${l.description}\n` +
...
/**
 * GST on a purchase is OFF by default.
 *
 * Munish's daily buying is milk, chicken, fish, vegetables — local unregistered
 * vendors who give a handwritten slip with no tax on it. Defaulting tax ON
 * overstated every one of those expenses. A formal GST invoice is the exception,
 * and he presses the toggle for it.
 */
export const PURCHASE_GST_PCT = Number(process.env.PURCHASE_GST_PCT) || 18;

export function applyPurchaseGst(lines, on) {
  const pct = on ? PURCHASE_GST_PCT : 0;
  return normalizeLines(lines).map((l) => ({ ...l, gst_pct: pct }));
}
