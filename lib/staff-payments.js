 ...
 *
 * SERVICE ROLE, deliberately. The rest of this app reaches Supabase with the
 * ANON key, and every older table carries a `for all using (true)` policy — so
 * that key, which ships inside the browser bundle and is readable by anyone who
 * opens devtools, can read and write those tables directly. Wage data is not
 * going on that pile. staff_payments keeps RLS on with NO policy, which locks
 * out the anon key entirely, and every call here goes through the server-side
 * service role. Nothing in this file is ever imported by a client component.
 */
