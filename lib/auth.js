...
/**
 * True only when ADMIN_PIN is set AND differs from MANAGER_PIN. When it isn't,
 * checkPin(pin, "admin") accepts the manager PIN too, so the owner PIN carries
 * no information the staff PIN doesn't — nothing may be inferred from it.
 */
export function hasDistinctAdminPin() {
  const manager = String(process.env.MANAGER_PIN || "1234").trim();
  const admin = String(process.env.ADMIN_PIN || "").trim();
