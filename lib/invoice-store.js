...
export async function editBill(billId, patch) {
  ...
  const { lines: normLines, subtotal, tax_total, grand_total } = computeTotals(
    lines.map((l) => ({
    ...
    { applyGst, inclusive: Array.isArray(patch.lines) && patch.lines.length ? inclusive : false }
  );
  ...
  if (billErr && /gst_inclusive/i.test(billErr.message || "")) {
