...
export async function buildInvoicePdf(bill) {
  ...
  // Card payment fee. It sits BELOW the grand total, outside the tax working,
  // because it is a recovery of the bank's charge on the swipe and not a supply
  // — so no GST is computed on it and it never enters the taxable value. The
  // guest pays the payable line; the tax invoice above it is unaffected.
  const cardFee = Math.round((Number(bill.card_fee_inr) || 0) * 100) / 100;
  const feePct = Number(bill.card_fee_pct) || 0;
  const payable = Math.round((grand + cardFee) * 100) / 100;
  if (cardFee > 0.009) {
    text(
      feePct
        ? `Add: Card payment fee @ ${String(feePct).replace(/\.00$/, "").replace(/(\.\d)0$/, "$1")}%`
        : "Add: Card payment fee",
      col.particular, y, 9, false, BRAND.muted
    );
    text(`(+) ${money(cardFee)}`, col.amt, y, 9, false, BRAND.muted);
    y -= 12;
    text(
      "Bank charge on card settlement — not a taxable supply, no GST applies.",
      col.particular, y, 7, false, BRAND.muted
    );
    y -= 16;
    text("Amount Payable (by card)", col.particular, y, 11, true, BRAND.terracotta);
    text(money(payable), col.amt, y, 11, true, BRAND.terracotta);
    y -= 20;
  }
  ...
  if (paid > 0) {
    ...
    y -= 15;
    // Owed is measured against the payable figure, so a card bill settled in
    // full does not print a balance equal to the fee.
    const balance = Math.round((payable - paid) * 100) / 100;
    if (balance > 0.009) {
  ...
  y -= 12;
  // Words follow what is actually chargeable, so they agree with the last
  // figure on the page. Identical to the grand total when there is no card fee.
  text(amountInWords(payable), margin, y, 9, true, BRAND.deepEarth);
  y -= 12;
  text(
