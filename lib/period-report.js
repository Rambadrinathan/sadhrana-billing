...
export async function getPeriodReport(from, to) {
  ...
  const [billsRes, expRes, attRes, staffRes] = await Promise.all([
    supabase
      .from("bills")
      .select(
    ...
    selectTolerant(
      supabase,
      "staff",
      "id, name, active, sort_order, daily_rate_inr",
  ...
  return {
    ...
    people: buildPeople(attendance, roster, range, countedThrough, staffRes.rich),
 ...
 */
export function buildPeople(attendance, roster, range, countedThrough, wagesAvailable) {
  ...
  const rows = [...byName.values()]
    .map((r) => {
      ...
      const payable = r.dailyRate != null ? round2(daysWorked * r.dailyRate) : null;
      const { byDate, ...rest } = r;
      return {
        ...rest,
  ...
  return {
    ...
    totalPayable: round2(
      rows.reduce((s, r) => s + (r.payable || 0), 0)
    ),
    ratesMissing: rows.filter((r) => r.dailyRate == null).length,
    wagesAvailable,
  };
}
